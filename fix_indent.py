import re

with open('/home/joelpp/Mechanic_Workshop/addons/taller_mecanico_portal/controllers/main.py', 'r') as f:
    lines = f.readlines()

in_func = False
for i, line in enumerate(lines):
    if line.startswith('    def taller_orden_propuesta_crear'):
        in_func = True
        continue
    
    if in_func:
        # Check if the function has ended (next top-level or method definition)
        if line.startswith('    @http.route') or line.startswith('    def '):
            break
            
        # We need to indent lines between '        if not user.exists():' and '        return {'success': True}'
        if i >= 921 and i <= 1003:
            if line.startswith('        '):
                lines[i] = '    ' + line
            elif line.startswith('    '):
                lines[i] = '    ' + line
            elif line.strip() == '':
                pass
            else:
                lines[i] = '    ' + line

with open('/home/joelpp/Mechanic_Workshop/addons/taller_mecanico_portal/controllers/main.py', 'w') as f:
    f.writelines(lines)
